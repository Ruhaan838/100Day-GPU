...
__global__ void conv_1d_shared(const float *in, float* out, int n){
    int thread_idx = threadIdx.x;
    int i = blockDim.x * blockIdx.x + thread_idx;

    __shared__ float shared_mem[32 + Wigths_size - 1];

    if (i < n)
        shared_mem[thread_idx + Wigths_size / 2] = in[i];
    
    if (thread_idx < Wigths_size / 2){
        int left_idx = blockIdx.x * blockDim.x - (Wigths_size / 2) + thread_idx;
        if (left_idx >= 0)
            shared_mem[thread_idx] = in[left_idx];
        else
    ...
    if (thread_idx < Wigths_size / 2){
        int right_idx = blockIdx.x * blockDim.x + blockDim.x + thread_idx;
        int temp_idx = thread_idx + blockDim.x + Wigths_size / 2;
        if (right_idx < n)
            shared_mem[temp_idx] = in[right_idx];
        else
            shared_mem[temp_idx] = 0.0f;
    ...
    if (i < n){
        ...
        for (int k = 0; k < Wigths_size; k++){
            int idx = thread_idx + k;
            if((i + k - Wigths_size / 2) >= 0 && (i + k - Wigths_size / 2) < n){
                result += shared_mem[idx] * wigths[k];
            }
        }
        out[i] = result;
    }
...
int main(){
    ...
    conv_1d_shared<<<dimGrid, dimBlock>>>(din, dout, n);
