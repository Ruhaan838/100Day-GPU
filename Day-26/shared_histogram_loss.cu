...
__global__ void compute_histogram_probs(float* values, float* support, float* probs, 
    ...
    extern __shared__ float support_shared[];

    if (threadIdx.x < n_bins + 1) {
        support_shared[threadIdx.x] = support[threadIdx.x];
    }
    __syncthreads();
    ...
    for (int i = 0; i < n_bins; i++){
        float cdf_right = erff((support_shared[i + 1] - value) / sigma_times_sqrt_two);
        float cdf_left = erff((support_shared[i] - value) / sigma_times_sqrt_two);
...
extern "C" void hlgauss_loss(float* d_values, float* d_support, float* d_probs, int n_values, int n_bins, float sigma){
    ...
    int shared_mem_size = (n_bins + 1) * sizeof(float);

    compute_histogram_probs<<<grid_size, block_size, shared_mem_size>>>(d_values, d_support, d_probs, n_values, n_bins, sigma, sigma_times_sqrt_two);
}
