...
void apply_rope(
    ...
    hipLaunchKernelGGL(RoPE_Kernel, grid_size, block_size, 0, 0,
        d_queries, d_keys,
...
int main() {
    ...
    hipMalloc(&d_queries, bytes);
    hipMalloc(&d_keys, bytes);

    hipMemcpy(d_queries, h_queries, bytes, hipMemcpyHostToDevice);
    hipMemcpy(d_keys, h_keys, bytes, hipMemcpyHostToDevice);
    ...
    hipMemcpy(h_queries_result, d_queries, bytes, hipMemcpyDeviceToHost);
    hipMemcpy(h_keys_result, d_keys, bytes, hipMemcpyDeviceToHost);
