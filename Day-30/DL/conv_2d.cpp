...
#define HIP_MAX_NUM_THREADS 1024
...
void ConvForward(float* input, float* weights, float* output, 
    ...
    hipMalloc(&input_unrolled, unrolled_size);
    ...
    for (int n = 0; n < batch_size; n++){
        ...
        hipLaunchKernelGGL(unrollKernel, dim3(unroll_blocks), dim3(BLOCK_SIZE), 0, 0,
            input_n, input_unrolled, input_channels, input_height, input_width,
            kernel_size, output_height, output_width);

        hipLaunchKernelGGL(convKernel, dim3(conv_blocks), dim3(BLOCK_SIZE), 0, 0,
            input_unrolled, weights, output_n, output_size, num_filters, filter_size);

        hipDeviceSynchronize();
    }

    hipFree(input_unrolled);
}
...
int main() {
    ...
    hipMemset(d_output, 0, output_size);
