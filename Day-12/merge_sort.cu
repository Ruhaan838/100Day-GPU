...
int main(){
    ...
    cudaMalloc(&dc, (N + M) * sizeof(int));

    cudaMemcpy(da, A, N * sizeof(int), cudaMemcpyHostToDevice);
    cudaMemcpy(db, B, M * sizeof(int), cudaMemcpyHostToDevice);

    dim3 block_dim(256);
    dim3 grid_size((N + M + block_dim.x - 1) / block_dim.x);

    parallel_merge<<<grid_size, block_dim>>>(da, db, dc, N, M);

    cudaMemcpy(C, dc, (N + M) * sizeof(int), cudaMemcpyDeviceToHost);
